...
- The collectible musette is bundled via an automatic **BxGy** discount ("Founder's Edition Musette", ACTIVE — buy 8 of the two coffees, get 1 musette at 100% off). Its product should be ACTIVE but **not** published to the Online Store channel (0 inventory, not standalone-purchasable); it is not a storefront SKU. ⚠️ **Drift — Aug 13, 2026:** it *is* currently published to Online Store (and Point of Sale, and Shop). Not buyable at 0 inventory, but unpublish it from Online Store before launch.
...
- **No-WebGL fallback (Aug 2026):** the loader is a full-screen cover, so anything that aborts `fox-meyer-homepage.js` before `reveal()` leaves the site stuck on the pulsing logo. It used to: `new THREE.WebGLRenderer` throws when the browser can't grant a WebGL context (hardware acceleration off, GPU context limit, blocked CDN → `THREE` undefined). The script now probes for `THREE`, wraps the renderer in try/catch, handles `webglcontextlost`, and degrades to `noThreeD()` — canvas removed, `.fm-no-3d` on `.fm-page` (collapses `.fm-hero-spacer`), page otherwise intact. Two watchdogs (`load` + 6s) reveal the loader no matter what. **Don't reintroduce work between the loader and `reveal()` that can throw.**
...
Theme is translation-ready (July 2026). **Status as of Aug 13, 2026 — the flip is done, but only half of it:** French **is** the store default (primary + published) and English is **NOT published**. The store is FR-only, so the native FR/EN toggle below renders nothing (it needs two published locales). Publish EN in Translate & Adapt if the toggle is wanted. How it's wired:
...
## Notes
- **Paused Aug 13, 2026.** Development is on hold and the Shopify plan is being paused/cancelled. **[`RESTART.md`](RESTART.md) is the pick-up point** — full live-store snapshot (products, IDs, shipping rates, discount config, pages, locales, themes), the drift list, and a restart checklist. Read it first; the Shopify-side state it records exists nowhere else. Shopify Files images are backed up in [`shopify-files-backup/`](shopify-files-backup/).
